...
void main (void) { 
        ...
        for (int i = 0; i < MAX_LIGHTS; i++) {
                ...
                float spotEffect = dot(normalize(spotlightdir), normalize(-lightDirection));
                ...
                if (lambertian > 0.0) {
                        ...
                        specular = vec4(pow(specAngle, 0.3*gl_FrontMaterial.shininess));
                        specular = gl_FrontLightProduct[i].specular * specular * SPECULAR_CONSTANT;

        		vec4 pos = gl_LightSource[i].position;
        		vec4 vvvv = vec4(vertexModelview, 0);
                        ...
                        finalColor += (diffuse + specular) * att * q;
                }
