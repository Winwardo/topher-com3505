...
public class Light implements Cloneable {
    ...
    // If w is 0, then it is a directional light (at infinite distance in the
    // given vector direction).
    // If w is 1, then it is a positional light (at the given position in the
    // scene, and emitting in all directions).
    ...
    // If it is 0.1,0.1,0.1, then a spotlight effect will be added to the
    // ambient material value for any object,
    // which can result in the spotlight appearing on polygons that are pointing
    ...
    public void makeSpotlight(float[] direction, float angle) {
        if (position[3] != 1) {
            System.out.println(
                "Error. Position[3] needs to be 1 for a spotlight. Will now change.");
            position[3] = 1;
        }
        spotlight = true;
    ...
    public void use(GL2 gl, GLUT glut, boolean show) {
        if (switchedOn) {
            ...
            if (show) {
                if (position[3] == 1) { // Normally you wouldn't use == to
                                        // compare two floats
                                        // but in this case I know 0 and 1 are
                                        // represented exactly.
